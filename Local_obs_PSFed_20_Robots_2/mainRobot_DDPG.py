...
def learning():
    FL_params = Arguments()
    LR_A = FL_params.LR_A  # learning rate for actor
    LR_C = FL_params.LR_C  # learning rate for critic
    numberOfRobots = FL_params.N_robotics
    ...
    for i in range(numberOfRobots):
        Robot0 = Agent(f'actor{i}', f'critic{i}', f'actor{i}_target', f'critic{i}_target',
                       alpha=LR_A, beta=LR_C,input_dims=[148],tau=0.001, n_actions=8)
    ...
    for i in range(numberOfAPs):
        AP1 = Agent(f'actor{i}_AP', f'critic{i}_AP', f'actor{i}_AP_target', f'critic{i}_AP_target',
                    alpha=LR_A,beta=LR_C,input_dims=[357], tau=0.001, n_actions=8)
        APs.append(AP1)
            ...
            for move in range(FL_params.move_num):
                ...
                if np.random.random() < epsilon_move:  #贪婪探索
                    for x in range(numberOfRobots):
                        act_robot_moves[x] = np.random.uniform(-1.0, 1.0), np.random.uniform(-1.0, 1.0)
                else:
                    for x in range(numberOfRobots):
                        act_robot_moves[x] = robot_movers[x].choose_action(Local_Observaion[x])
                ...
                for k in range(1):
                    for j in range(FL_params.local_epoch):
                        ...
                        AP_observation = obs[numberOfRobots]  # the last observation is for access points
                        print("初始训练的")
                        print(AP_observation.shape[0])
                        for R in range(numberOfRobots):
                            Local_Observaion.append(obs[R])
                        print(Local_Observaion[0].shape[0])
                        epsilon -= 1 / Explore
                        a += 1
                        ...
                        if np.random.random() <= epsilon:
                            ...
                            for y in range(numberOfRobots):
                                act_robots[y] = robotics[y].choose_action(Local_Observaion[y])
                            for y in range(numberOfAPs):
                                act_APs[y] = APs[y].choose_action(AP_observation)

                        act_robots_co = np.concatenate(act_robots)
                        # print("输出robot_moves")
                        # print(act_robot_moves)
                        act_robot_moves_co = np.concatenate(act_robot_moves)
                        act_APs_co= np.concatenate(act_APs)

                        new_state, reward_Robot, reward_AP, done, info, accept, AoI, energy, posX0, posY0, posX1, posY1 = env.step_task_offloading(act_APs_co, act_robots_co, act_robot_moves_co)
...
def unlearning(robotics,robot_movers,APs,old_GMs, old_CMs,FL_params):
    ...
    old_global_models = copy.copy(old_GMs)
    old_client_models = copy.copy(old_CMs)
    gm_len=len(old_global_models)
    print("全局模型的长度:"+str(len(old_GMs)))
    ...
    sess_temp = tf.compat.v1.Session()
    ...
    actor_move = Actor(FL_params.LR_A, 2, 'tam', [148], sess_temp,
                   32, 32, 1, batch_size=64, chkpt_dir='/checkpoint')
    critic_move = Critic(FL_params.LR_C, 2, 'tcm', [148], sess_temp,
                     32, 32, chkpt_dir='/checkpoint')

    #3、重置经验池+重置初始参数
    for ii in range(len(robotics)):
        robotics[ii].memory.clear()  #重置经验池
        robotics[ii].actor.params=actor1.params
        robotics[ii].critic.params=critic1.params
        robotics[ii].target_actor.params=actor1.params
        robotics[ii].target_critic.params=critic1.params
        #robotics[ii].actor.params=old_client_models[0][ii] #赋予第一次的值
    for ii in range(len(robot_movers)):
        robot_movers[ii].memory.clear()
        robot_movers[ii].actor.params = actor_move.params
        robot_movers[ii].critic.params = critic_move.params
        robot_movers[ii].target_actor.params = actor_move.params
        robot_movers[ii].target_critic.params = critic_move.params
    for ii in range(len(APs)):
        APs[ii].reset(f'actor{ii}_AP1', f'critic{ii}_AP1', f'actor{ii}_AP_target1', f'critic{ii}_AP_target1',alpha=FL_params.LR_A,
                beta=FL_params.LR_C, input_dims=[346], tau=0.001, n_actions=8)
    ...
    #训练用到的参数
    Server_robot = Federated_Server(numberOfRobots, name_actor='server_actor_unlearn', name_critic='server_critic_unlearn',
                                    input_dims=[148],
                                    n_actions=8, layer1_size=32, layer2_size=32)
    Server_AP = Federated_Server_AP(name_actor='server_actor_AP_unlearn', name_critic='server_critic_AP_unlearn', input_dims=[346],
                                    n_actions=8, layer1_size=32, layer2_size=32)
    score_Robot = 0
    score_AP = 0
    score1_history = []
            ...
            for move in range(FL_params.move_num):
                ...
                for k in range(1):
                    for j in range(FL_params.local_epoch):
                        ...
                        if np.random.random() <= epsilon:
                            for y in range(numberOfRobots):
                                act_robots[y] = np.random.uniform(-1.0, 1.0), np.random.uniform(-1.0,
                                                                                                1.0), np.random.uniform(
                                    -1.0, 1.0), np.random.uniform(-1.0, 1.0), np.random.uniform(-1.0,
                                                                                                1.0), np.random.uniform(
                                    -1.0, 1.0), np.random.uniform(-1.0, 1.0), np.random.uniform(-1.0, 1.0)
                        ...
                        new_state, reward_Robot, reward_AP, done, info, accept, AoI, energy, posX0, posY0, posX1, posY1 = env.step_task_offloading(
                            act_APs_co, act_robots_co, act_robot_moves_co)
                        for re in range(numberOfRobots):
                    ...
                    if gn==0:
                        actor_robot_new = Server_robot.federation()
                        unlearn_global_models.append(actor_robot_new)
                    else :
                        actor_robot_new =unlearn_global_models[gn]
                    if gn < gm_len-1 :
                        ...
                        new_GM=unlearn_global_models[gn]
                        #actor_robot_new = Server_robot.federation()
                    actor_AP_new = Server_AP.federation()
                    gn=gn+1
...
def unlearning_step_once(old_client_models, new_client_models, global_model_before_forget, global_model_after_forget):
    ...
    for layer in range(len(global_model_before_forget)):
...
if __name__ == '__main__':
    ...
    print("近似遗忘训练总用时：" + str(time_unlearn))
